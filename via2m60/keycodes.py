...
def MODS(*args):
    MAP = {LCTRL: 1, LSHIFT: 2, LALT: 4, LGUI: 8,
...
def ACTION(kind, param): return (kind << 12) | param


def MODS_KEY(mods, key): return ACTION(
    ACT_MODS, (mods << 8) | get_action_code(key))
def MODS_TAP(mods, key): return ACTION(
    ACT_MODS_TAP, (mods << 8) | get_action_code(key))


def MOUSEKEY(key): return ACTION(ACT_MOUSEKEY, key)


def LAYER_BITOP(op, part, bits, on): return ACTION(
    ACT_LAYER, op << 10 | on << 8 | part << 5 | (bits & 0x1f))


def LAYER_BIT_XOR(part, bits, on): return LAYER_BITOP(
    OP_BIT_XOR, part, bits, on)


def LAYER_INVERT(layer, on): return LAYER_BIT_XOR(
    layer/4,   1 << (layer % 4),  on)


def LAYER_TOGGLE(layer): return LAYER_INVERT(layer, ON_RELEASE)


def LAYER_TAP(layer, key=NO): return ACTION(
    ACT_LAYER_TAP, (layer << 8) | get_action_code(key))


def LAYER_TAP_TOGGLE(layer): return LAYER_TAP(layer, OP_TAP_TOGGLE)
def LAYER_MODS(layer, mods): return LAYER_TAP(layer, 0xC0 | mods)


def ACTION_USAGE_SYSTEM(n): return ACTION(ACT_USAGE, n)
def ACTION_USAGE_CONSUMER(n): return ACTION(ACT_USAGE, 1 << 10 | (n))
